...
# Output directory relative to the KARMA project root (KARMA/output/)
_OUTPUT_DIR = Path(__file__).resolve().parent.parent.parent / "output"
_SUMMARIES_DIRNAME = "summaries"
_LOCAL_KG_DIRNAME = "local_kg"
_UNIFIED_KG_DIRNAME = "unified_kg"


def _resolve_output_root(output_dir: Optional[Union[str, Path]] = None) -> Path:
    return Path(output_dir) if output_dir is not None else _OUTPUT_DIR


def _ensure_output_layout(base_output_dir: Path) -> tuple[Path, Path, Path]:
    summaries_dir = base_output_dir / _SUMMARIES_DIRNAME
    local_kg_dir = base_output_dir / _LOCAL_KG_DIRNAME
    unified_kg_dir = base_output_dir / _UNIFIED_KG_DIRNAME

    for path in (base_output_dir, summaries_dir, local_kg_dir, unified_kg_dir):
        path.mkdir(parents=True, exist_ok=True)

    return summaries_dir, local_kg_dir, unified_kg_dir
...
class KARMAPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            base_url: Optional API base URL (default: OpenRouter)
    ...
    def process_document(
        ...
        Args:
            ...
            output_dir: Root directory for KARMA outputs. If None, defaults
                        to KARMA/output/.
        ...
        try:
            ...
            try:
                ...
                summaries_dir, _, _ = _ensure_output_layout(output_root)
                source_name = Path(source).stem if isinstance(source, (str, Path)) and Path(source).exists() else "document"
                summaries_path = summaries_dir / f"{source_name}_summaries.json"
                with open(summaries_path, "w", encoding="utf-8") as _f:
                    json.dump(summaries, _f, ensure_ascii=False, indent=2)
                self._log(f"Summaries saved to {summaries_path}")
            except Exception as _e:
                logger.warning(f"Failed to save summaries: {_e}")
    ...
    def export_document_knowledge_graph(
        self,
        result: IntermediateOutput,
        output_path: Union[str, Path]
    ) -> Dict:
        """
        Export a single document's knowledge graph to a separate JSON file.

        Args:
            result: IntermediateOutput from processing a single document
            output_path: Path to save the per-document knowledge graph

        Returns:
            Dictionary representation of the document's knowledge graph
        """
        doc_kg = KnowledgeGraph()

        # Add aligned entities
        for entity in result.aligned_entities:
            doc_kg.add_entity(entity)

        # Add integrated triples
        for triple in result.integrated_triples:
            doc_kg.add_triple(triple)

        # Attach document metadata
        if result.metadata:
            doc_kg.metadata = result.metadata.to_dict()

        output_path = Path(output_path)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        doc_kg.save_to_file(str(output_path))
        self._log(f"Document knowledge graph saved to {output_path}")
        return doc_kg.to_dict()
    ...
    def process_batch(
        ...
        Args:
            ...
            output_dir: Root directory for KARMA outputs. Per-paper summaries
                        are written to summaries/, per-paper graphs to
                        local_kg/, and aggregate graphs can be stored in
                        unified_kg/. If None, defaults to KARMA/output/.

        Returns:
            List of processing results
        """
        output_dir = _resolve_output_root(output_dir)
        _, local_kg_dir, _ = _ensure_output_layout(output_dir)
        ...
        for i, source in enumerate(sources):
            ...
            try:
                ...
                # Save per-document knowledge graph
                source_name = Path(source).stem if Path(source).exists() else f"document_{i}"
                doc_kg_path = local_kg_dir / f"{source_name}_kg.json"
                self.export_document_knowledge_graph(result, doc_kg_path)
